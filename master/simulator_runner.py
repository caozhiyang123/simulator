...
class SimulatorRunner:
    ...
    def __init__(self, simulator_dir: str, production_dir: str = ""):
        ...
        self._model_results: dict[str, dict] = {}  # {json_filename: {fields...}}
    ...
    def _parse_log_line(self, line: str) -> None:
        ...
        模拟器可能多线程输出，同一个 SPIN COUNT 会出现多次。
        只有当 SPIN COUNT 严格递增时才视为新一轮数据。
        只有收齐全部 7 个字段的完整快照才存入 history。
        """
        stripped = line.strip()
        ...
        # Parse data lines (KEY:VALUE format)
        if ":" in stripped and self._current_model:
            ...
            if len(key_val) == 2:
                ...
                try:
                    ...
                    if key == "SPIN COUNT":
                        sc = int(val)
                        # Only accept strictly increasing SPIN COUNT as new round
                        if sc > self._last_spin_count:
                            self._finalize_current_snapshot()
                            self._current_model_data = {}
                            self._last_spin_count = sc
                        # Same SPIN COUNT: continue collecting (overwrite with latest)
    ...
    def get_status(self) -> dict:
        """返回当前本地模拟器状态，包含 per-model 实时结果。

        Returns:
            dict 包含:
            - status: idle|running|completed|error
            - job_id: 当前任务 ID
            - models_completed: 已完成的 model 数
            - models_total: 总 model 数
            - model_results: {json_filename: {fields...}}
            - error: 错误信息
        """
        with self._lock:
